...
public class GameArenaOrb extends View implements View.OnClickListener, GameStateObserver {
   ...
   */
  public void initView(BombValues bombValues, GameCellInfo gameCellInfo,
                       GamePlayerInfo gameCurrentPlayer) {
  ...
  public void onClick(View v) {
    if (gameCellInfo.getGamePlayerInfo() != null &&
        gameCurrentPlayer != gameCellInfo.getGamePlayerInfo()) {
      return;
    }
  ...
  protected void onDraw(Canvas canvas) {
    ...
    paintToDraw.setColor(
        CommonUtils.getColorByPlayerColor(gameCurrentPlayer.getPlayerColor(), getContext()));
    paintToDraw.setStyle(Paint.Style.STROKE);
    paintToDraw.setStrokeWidth(
        getContext().getResources().getDimension(R.dimen.game_screen_stroke_dimen));
    ...
    //Draw the Orbs
    paintToDraw.setColor(
        CommonUtils.getColorByPlayerColor(gameCellInfo.getGamePlayerInfo().getPlayerColor(),
            getContext()));
  ...
  private void drawOneOrb(Canvas canvas) {
    switch (gameBombType) {
      ...
      case TRIANGLE:
        ...
        Point p2 = new Point(getMeasuredWidth() / 2 - triangleSide / 2,
            getMeasuredHeight() / 2 + triangleSide / 2);
        Point p3 = new Point(getMeasuredWidth() / 2 + triangleSide / 2,
            getMeasuredHeight() / 2 + triangleSide / 2);
  ...
  private void drawTwoOrbs(Canvas canvas) {
    switch (gameBombType) {
      case CIRCLE:
        ...
        break;
      case TRIANGLE:
        Point p1 = new Point(getMeasuredWidth() / 2 - triangleSide / 2,
            getMeasuredHeight() / 2 - triangleSide / 2);
        Point p2 = new Point(getMeasuredWidth() / 2 - triangleSide,
            getMeasuredHeight() / 2 + triangleSide / 2);
        ...
        Point p4 = new Point(getMeasuredWidth() / 2 + triangleSide / 2,
            getMeasuredHeight() / 2 - triangleSide / 2);
        Point p5 = new Point(getMeasuredWidth() / 2 + triangleSide,
            getMeasuredHeight() / 2 + triangleSide / 2);
  ...
  private void drawThreeOrbs(Canvas canvas) {
    switch (gameBombType) {
      case CIRCLE:
        canvas.drawCircle(getMeasuredWidth() / 2 - circleRadius,
      ...
      case TRIANGLE:
        ...
        Point p5 = new Point(getMeasuredWidth() / 2 - triangleSide,
            getMeasuredHeight() / 2 + triangleSide);
        ...
        Point p9 = new Point(getMeasuredWidth() / 2 + triangleSide,
            getMeasuredHeight() / 2 + triangleSide);
