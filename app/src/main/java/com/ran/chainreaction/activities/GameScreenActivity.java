...
public class GameScreenActivity extends ActionBarActivity
    implements ExitAlertDialogCreator.ButtonOnClickListener,
  ...
  private void initView() {
    ...
    if (isOnline) {
      ...
      savedGamePlayEntity =
          ChainReactionDBOpsHelper.getDBInstance(this).retrieveGameSession(savedGameId);
      ...
      gameName.setText(getResources().getString(R.string.game_screen_gameTitle) +
          GameInfoUtility.generateGameName(this));
    }
  ...
  public void onBackPressed() {
    ...
    if (mBackDialog == null || !mBackDialog.isShowing()) {
      mBackDialog =
          ExitAlertDialogCreator.createDialog(mBackDialogEntries, mBackDialogTitle, this, true);
      mBackDialog.show();
    } else {
      super.onBackPressed();
    }
    ...
    try {
      ...
      switch (tag) {
        ...
        case RESTART_TAG:
          ...
          break;
        case SAVE_EXIT_TAG:
          ChainReactionDBOpsHelper.getDBInstance(this)
              .addCurrentGame(GamePlayLogic.getGameInstance().getGamePlaySession(),
  ...
  private void initialiseGameSession() {
    ...
    if (isOnline) {
      ...
      GameSizeBoxInfo sizeBoxInfo = GameInfoUtility.generateGameSizeBoxInfo(this,
          ChainReactionPreferences.getGridSizePreference(this));
      Gson gson = new Gson();
      Type type = new TypeToken<ArrayList<GamePlayerInfo>>() {
      }.getType();

      ArrayList<GamePlayerInfo> gamePlayerInfos =
          gson.fromJson(GamePreferenceUtils.getPlayerInfoGame(this), type);
      ...
      gamePlaySession = new GamePlaySession(gamePlayerInfos,
          ...
          sizeBoxInfo,
          GameInfoUtility.generateGameCellInfo(this, sizeBoxInfo.getX_boxes(),
              sizeBoxInfo.getY_boxes(), null));
      updateGameTurnState(currentPlayer);
    }
  ...
  private void timerInitialization(final long futureTime) {
    countDownTimer = new CountDownTimer(futureTime, TIME_INTERVAL) {
      ...
      public void onTick(long millisUntilFinished) {
        gameTimer.setText(GameInfoUtility.generateTimeFormat(
            futureTime + currentGamePrevTimeElapsed - millisUntilFinished));
